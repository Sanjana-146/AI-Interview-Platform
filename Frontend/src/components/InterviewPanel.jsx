...
export default function InterviewPanel() {
  ...
  useEffect(() => {
    ...
    if (warnings > MAX_WARNINGS) {
      ...
      alert(`Interview Terminated: ${reason}. You have exceeded the maximum number of warnings.`);
    } else {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden) {
        handleWarning("Do not switch tabs or minimize the window during the interview.");
      }
    };
  ...
  const handleLeave = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.message || 'Failed to get analysis from the server.');
      }
